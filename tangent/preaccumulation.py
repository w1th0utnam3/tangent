  ...
  if scalar_seed is not None:
    ...
    if isinstance(obj, numpy.ndarray):
      ...
      for i in range(len(obj)):
        for seed, reset in _unit_seed_gen(obj[i]):
          obj[i] = seed
          yield obj, reset

    elif isinstance(obj, dict):
      for k in obj.keys():
        for seed, reset in _unit_seed_gen(obj[k]):
          obj[k] = seed
          yield obj, reset
...
def unit_seed_directions(obj):
  obj_grad = utils.init_grad(obj)
  for direction, is_reset in _unit_seed_gen(obj_grad):
    if not is_reset:
      yield direction
